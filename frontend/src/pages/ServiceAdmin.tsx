...
export function ServiceAdmin() {
  ...
  const fetchTickets = async (userRole?: string) => {
    ...
    try {
      ...
      const mappedData: Ticket[] = rawData.map((t: any) => ({
        _id: String(t.id),
        nome: t.nomeSolicitante,
        local: t.local || 'Não informado',
        tipo: t.tipoProblema,
        descricao: t.descricao,
        status: t.status.toLowerCase(),
        justificativa: t.justificativa,
        materialUtilizado: t.materialUtilizado,
        imagemUrl: t.anexoUrl ? `http://localhost:3000${t.anexoUrl}` : undefined, 
        _createdAt: t.dataAbertura,
        setor: t.setorResponsavel ? t.setorResponsavel.toLowerCase() : 'outros'
      }));
      ...
      if (roleLimpa === 'ti') {
        filteredData = mappedData.filter(t => t.setor === 'ti');
      } 
      else if (roleLimpa === 'manutencao') {
        filteredData = mappedData.filter(t => t.setor === 'manutencao');
      }
      ...
      alert("Erro ao carregar lista de chamados.");
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    ...
    if (userStr) {
      ...
      // --- CORREÇÃO AQUI: Força minúsculo para garantir a comparação ---
      if (user.role) {
  ...
  const updateStatus = async (id: string, newStatus: 'em_andamento' | 'aguardando' | 'concluido' | 'cancelado') => {
    ...
    try {
      ...
      await api.patch(`/tickets/${id}`, patchData);
  ...
  const handlePrintReport = () => {
    ...
    const titulos = { 
      ...
      aguardando: 'AGUARDANDO / EM ESPERA',
  ...
  const filteredTickets = tickets.filter(t => {
    ...
    const matchesSearch = t._id.includes(searchTerm) || t.nome.toLowerCase().includes(searchTerm.toLowerCase()) || t.local.toLowerCase().includes(searchTerm.toLowerCase());
        ...
        <div className="flex-1 space-y-6">
          <div className="flex justify-between items-center mb-2">
            ...
            <div className="flex items-center gap-4">
              ...
              {/* --- BOTÃO CORRIGIDO AQUI: Verificando role com lowercase --- */}
          ...
          {selectedTicket ? (
            <div className="bg-white rounded-lg shadow-md border border-gray-200 overflow-hidden animate-fade-in">
              <div className="bg-gray-50 px-6 py-4 border-b border-gray-200 flex justify-between items-center">
                <div>
                  <h2 className="text-xl font-bold text-gray-800">Ordem #{selectedTicket._id}</h2>
              ...
              <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="md:col-span-2 space-y-6">
                  ...
                  {selectedTicket.imagemUrl && (
                    <div>
                      <h3 className="font-bold text-gray-800 mb-2">Anexo</h3>
                      <img src={selectedTicket.imagemUrl} alt="Anexo do chamado" className="rounded-lg border border-gray-200 max-h-80 object-contain bg-gray-100" />
                    </div>
                  )}
                </div>
                  ...
                  <div className="flex items-center gap-4">
                    <div className={`w-12 h-12 rounded-lg flex items-center justify-center 
                      ${ticket.status === 'pendente' ? 'bg-blue-100 text-blue-600' : 
                        ...
                        ticket.status === 'aguardando' ? 'bg-orange-100 text-orange-600' :
                    ...
                    <div>
                      <h4 className="font-bold text-gray-800 flex items-center gap-2">
                        {ticket.nome} 
                        <span className="text-xs font-normal text-gray-400 bg-gray-100 px-2 py-0.5 rounded">#{ticket._id}</span>
                      </h4>
